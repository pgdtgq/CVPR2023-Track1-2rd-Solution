...
def inference_on_dataset(model, data_loader, evaluator, flip_horizontal=False, moe_group=None):
    """
    ...
    with paddle.no_grad():
        for idx, inputs in enumerate(data_loader):
            ...
            if flip_horizontal:
                ...
                if 'fgvc' in inputs:
                    flips_inputs,ins = {},{}
                    ins['image'] =  inputs['fgvc']['image'].flip(axis=[3])
                    flips_inputs['fgvc'] =ins
                    flip_outputs = model(flips_inputs, moe_group) if moe_group is not None else model(flips_inputs)
                    outputs['fgvc'] = outputs['fgvc']  + flip_outputs['fgvc']  #翻转推理
