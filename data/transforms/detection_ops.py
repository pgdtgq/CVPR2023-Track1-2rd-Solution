...
class BaseOperator(object):
    ...
    def __call__(self, sample, context=None):
        ...
        if isinstance(sample, Sequence) and self.__class__.__name__ == 'Decode':  #只有deocde走这个通路
            ...
            # if 'masaic' in sample and self.__class__.__name__ == 'RandomSelect':
            #     return sample
...
# class OneofSelect(BaseOperator):
#     """
#     Randomly choose a transformation between transforms1 and transforms2,
#     and the probability of choosing transforms1 is p.
...
#     def __init__(self, transforms_content):
#         super(RandomSelect, self).__init__()
#         num_transforms = len(transforms)
#         self.transforms_cls = []
#         for i in range(num_transforms):
#             self.transforms_cls.append( transforms_content[i])
#         self.transforms1 = Compose(transforms1)
#         self.transforms2 = Compose(transforms2)
#         self.p = p
...
#easy的图像增强用于马赛克数据扩增之后
class One_of_aug_onlyimg(BaseOperator):
    def __init__(self,):
        self.transform = A.Compose([  
                A.OneOf([  #改变颜色类
                        A.RGBShift((-10,10),(-10,10),(-10,10),p=1.0),
                        A.RandomBrightnessContrast(brightness_limit=0.1, contrast_limit=0.1, p = 1.0),
                        A.ColorJitter(brightness=0.1, contrast=0.1, saturation=0.1, hue=0.05, p=1.0),
                        ],p = 0.4),   
                A.OneOf([  #噪声类
                        A.Blur(blur_limit=3, p=0.25), 
                        A.GaussianBlur(blur_limit=[3,5], sigma_limit=[0,1], p = 0.25),
                        A.ISONoise(color_shift=(0.02,0.05), intensity=(0.1,0.3), p=0.15),  #ISO噪声
                        A.GaussNoise(var_limit=(10.0, 30.0), p=0.15),  #高斯噪声   
                        A.ImageCompression(quality_lower=40, quality_upper=90, p=0.2),  #压缩
                        ],p = 0.2),    
                ], p = 1.0)
    
    def apply(self, sample, context=None):  #数据扩增
        im = sample['image']
        augs = self.transform(image=np.uint8(im))
        sample['image'] = augs['image']
        return sample


#hard的图像增强
class One_of_aug_onlyimg_hard(BaseOperator):
    def __init__(self,):
        self.transform = A.Compose([  
                A.OneOf([  #改变颜色类（增加了随机天气）
                        A.RandomRain(drop_length=20, drop_width=1, drop_color=(200, 200, 200),   #drop_width雨的粗细
                                blur_value=3, brightness_coefficient=0.9, rain_type=None, always_apply=False, p=0.5), #增加雨
                        A.RandomFog(fog_coef_lower=0.1, fog_coef_upper=0.3, alpha_coef=0.25, always_apply=False, p=0.5),  #随机增加雾
                        A.RGBShift((-10,10),(-10,10),(-10,10),p=1.0),
                        A.RandomBrightnessContrast(brightness_limit=0.1, contrast_limit=0.1, p = 1.0),
                        A.ColorJitter(brightness=0.1, contrast=0.1, saturation=0.1, hue=0.05, p=1.0),
                        ],p = 0.4),  
