...
class Checkpointer(object):
    ...
    def load(self, path: str, checkpointables=None):
        ...
        self.model.set_state_dict(checkpoint_state_dict)  #直接装载参数？
        # missing_keys = []
        # unexpected_keys = []
        ...
        #             print('{} has {}, but {} has {}'.format(k, v.shape, k, checkpoint_state_dict[k].shape))
        ...
        #             print('{} has {}, but {} has {}'.format(k, v.shape, k, self.model.state_dict()[k].shape))
        ...
        for key in self.checkpointables if checkpointables is None else checkpointables:
            if key in checkpoint:  # pyre-ignore
                ...
                # if key == 'optimizer':
                #     checkpoint[key]['dymic_weight_moment1_0'] = paddle.zeros([3])  #增加的
                #     checkpoint[key]['dymic_weight_moment2_0'] = paddle.zeros([3])  #增加的
                #     checkpoint[key]['dymic_weight_beta1_pow_acc_0'] = paddle.zeros([1])  #增加的
                #     checkpoint[key]['dymic_weight_beta2_pow_acc_0'] = paddle.zeros([1])  #增加的
