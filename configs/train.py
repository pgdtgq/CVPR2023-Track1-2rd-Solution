    ...
    task_loaders=L(OrderedDict)(
        ...
        segmentation=L(build_segmentation_trainloader)(
            data_set=L(build_segmentation_dataset)(
                    ...
                    dataset_root=_root + '/datasets/attack_train_data/seg/',   #分割数据集路径
                    transforms=[
        ...
        fgvc=L(build_vehiclemulti_train_loader_lazy)(
            ...
            train_set=L(build_hierachical_softmax_train_set)(
                names = ("FGVCDataset",),
                train_dataset_dir = _root + '/datasets/attack_train_data/cls/trainval/',
                test_dataset_dir = _root + '/datasets/attack_test_data/cls/test/',
                train_label = _root + '/datasets/attack_train_data/cls/trainval.txt',
                test_label = _root + '/datasets/attack_test_data/cls/test.txt',
        ...
        trafficsign=L(build_cocodet_loader_lazy)(
            data_set=L(build_cocodet_set)(
                ...
                dataset_dir= _root + '/datasets/attack_train_data/dec/',
        ...
        task_loaders=L(OrderedDict)(
            segmentation=L(build_segmentation_trainloader)(
                data_set=L(build_segementation_test_dataset)(
                        dataset_name="BDD100K",
                        dataset_root=_root + '/datasets/attack_train_data/seg/', 
        ...
        task_loaders=L(OrderedDict)(
            fgvc=L(build_reid_test_loader_lazy)(
                test_set=L(build_hierachical_test_set)(
                    dataset_name = "FGVCDataset",  #分类数据集
                    train_dataset_dir = _root + '/datasets/attack_train_data/cls/trainval/',    #train和val一起用于训练
                    test_dataset_dir = _root + '/datasets/attack_test_data/cls/test/',
                    train_label = _root + '/datasets/attack_train_data/cls/trainval.txt',
                    test_label = _root + '/datasets/attack_test_data/cls/test.txt',
                    transforms=L(build_transforms_lazy)(
...
model=L(MultiTaskBatchFuse)(
    ...
    heads=L(OrderedDict)(
        ...
        fgvc=L(ClsHead)(
            neck = L(CrossLevelFeatureFuseModule)(  #跨层特征融合模块
                       hidden_dim=[192,384,768,1536],
                       transformer_block_nums=1, 
                       transformer_block_num_heads=2,   #所采用的transformer head数量
                       gate_T=0.1, 
                       gate_alpha=0, 
                       indices = [0,1,2,3]),
            embedding_size=1536,  
...
train.init_checkpoint = _root + '/pretrained/dino.pdmodel'  

train.output_dir = _root + '/outputs/train'   
