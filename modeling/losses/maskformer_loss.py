...
class maskFormerloss(nn.Layer):
    ...
    def loss_labels(self, outputs, targets, indices, num_masks):
        """Classification loss (NLL)  output网络的主输出   indices是前一步经过匈牙利算法得到的对应关系
        ...
        src_logits = outputs["pred_logits"]  # 1 100 20
        idx = self._get_src_permutation_idx(indices)  #indices[0]表示bs， indices[1]表示预测的100个ins中匹配到的ind
        target_classes_o = paddle.concat(
            [t["labels"][J] for t, (_, J) in zip(targets_cpt, indices_cpt)])  #indices_cpt中的J表示的所预测的目标对应的gt的ind
        target_classes = paddle.full(
            src_logits.shape[:2], self.num_classes, dtype='int64')  
        target_classes[idx] = target_classes_o

        loss_ce = F.cross_entropy(
            src_logits.transpose((0, 2, 1)).cast('float32'),  #1 20 100
            target_classes,  #target_classes.shape = 1 * 100
    ...
    def loss_masks(self, outputs, targets, indices, num_masks):
        ...
        src_masks = src_masks[src_idx]  #预测的100个instance对应的masks
        if src_masks.ndim == 2:
    ...
    def _get_src_permutation_idx(self, indices):
        batch_idx = paddle.concat(
            [paddle.full_like(src, i) for i, (src, _) in enumerate(indices)])  #src表示的100个预测中与gt相呼应的的ind
    ...
    def forward(self, logits, targets):
        #logits是一个字典
        #logits['pred_logits']:[1, 100, 20],100个目标 每个目标在20类上的发呢类
        #logits['pred_masks']:[1, 100, 160,90],100个目标  各个目标的mask 下采样16倍
        #targets['gt_masks']  mask标签 [1, 19, 640, 360]    1是batch_size
        #targets['gt_classes']  #表示一张图中所含有类别数量
        targets_cpt = []
        ...
        for target_per_image_idx in range(batch_size):
            ...
            #将每一张图的label和mask按照list保存
            targets_cpt.append({  #targets_cpt[0]['labels'].shape = 19 (此时里面还包含有ignore_ind)
                "labels": targets['gt_classes'][target_per_image_idx, ...],
                "masks": padded_masks,
                "image_path":  targets['image_path'][target_per_image_idx]
            })
        #下面这一步主要是删除ignore ind
        targets = []
        for item in targets_cpt:
            invalid_indices = paddle.nonzero(
                paddle.cast(item['labels'] == self.ignore_index, 'int64'))  ###找出对应有类别的 删除ignore的ind
        ...
        indices = self.matcher(logits_without_aux, targets)  #匹配损失是利用主输出

        num_masks = sum(len(t['labels']) for t in targets)  #多少个gt mask
        ...
        # print(losses['loss_ce'], losses['loss_mask'],loss['loss_dice'])
        # print(losses)
