...
class DETRLoss(nn.Layer):
    ...
    def __init__(self,
                 ...
                 use_imporved_der_loss = False,
                 change_loss_iter = 40000):
        r"""
        Args:
        ...
        self.efl = EqualizedFocalLoss(num_classes)  #后面会用到
        # self.count = 0
        ...
        self.use_imporved_der_loss = use_imporved_der_loss
        if self.use_imporved_der_loss:
            print('=======================================>use improved efl dinoloss!')
            self.change_loss_iter = change_loss_iter
    ...
    def _get_loss_class(self,
        ...
        if self.use_focal_loss:
            target_label_one_hot = F.one_hot(target_label,
                                     self.num_classes + 1)[..., :-1]
        
        # print('current_iter', current_iter)
        if self.use_focal_loss:
            if self.use_imporved_der_loss and current_iter is not None and current_iter >self.change_loss_iter:
                cls_loss = self.loss_coeff['class'] * self.efl(logits, target_label, target_label_one_hot)
            else:
                # print('current_iter', current_iter)
                cls_loss = self.loss_coeff['class'] * sigmoid_focal_loss(logits, target_label_one_hot, num_gts / num_query_objects)
        else:
            cls_loss = F.cross_entropy(logits, target_label, weight=self.loss_coeff['class'])
            
        
        return {name_class: cls_loss}
        ...
        for aux_boxes, aux_logits in zip(boxes, logits):
            ...
            loss_class.append(
                self._get_loss_class(aux_logits, gt_class, match_indices,
                                     bg_index, num_gts, postfix, current_iter)['loss_class' +
                                                                 postfix])
        ...
        total_loss.update(
            self._get_loss_class(logits[
                -1] if logits is not None else None, gt_class, match_indices,
                                 self.num_classes, num_gts, postfix, current_iter))
        total_loss.update(
        ...
        if self.aux_loss:
            total_loss.update(
                self._get_loss_aux(
                    ...
                    self.num_classes, num_gts, dn_match_indices, postfix,current_iter))
