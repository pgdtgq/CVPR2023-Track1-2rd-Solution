...
class MaskFormer(nn.Layer):
    ...
    def __init__(self,
        ...
        self.seghead = MaskFormerHead(in_channels, num_classes)
    ...
    def forward(self, x, inputs,current_iter = None):
        if self.use_neck:
            feats = self.neck(x)  
        else:
        ...
        dicts['res4'] = feats[2]
        dicts['res5'] = feats[3]
        x_shape = inputs['image'].shape[2:]  #输入的大小
        outputs = self.seghead(dicts)
        dicts = {}  #释放？
        
        if self.training:
            # labels = inputs['label'].astype('int64')
            return self.loss(outputs, inputs)
        else:
            mask_cls_results = outputs["pred_logits"]  # [2, 100, 20]
            mask_pred_results = outputs["pred_masks"]  # [2, 100, 160，90]
...
class Transformer(nn.Layer):
    ...
    def forward(self, src, mask, query_embed, pos_embed):
        ...
        bs, c, h, w = src.shape
        src = paddle.transpose(paddle.flatten(src, start_axis=2), (2, 0, 1)) #转换为token的形式
        pos_embed = paddle.transpose(  
            paddle.flatten(
                pos_embed, start_axis=2), (2, 0, 1))  #位置编码
        ...
        memory = self.encoder(
            ...
            pos=pos_embed)  # HWxNxC memory = src  #得到encoder的组合特征
        hs = self.decoder(
            tgt,
            memory,
...
class TransformerPredictor(nn.Layer):
    ...
    def forward(self, x, mask_features):
        pos = self.pe_layer(x)  #最后一层转换为256的维度 这里256就是token的embedding维度

        mask = None  # self.input_proj(x)将输入x转换为token  self.query_embed.weight查询矩阵，预测100个实例
        hs, memory = self.transformer(
            self.input_proj(x), mask, self.query_embed.weight, pos)

        out = {}
        if self.mask_classification:
            outputs_class = self.class_embed(hs)  #类别  6 * 1 *100*20 100个instance 20个cls 6层输出
            out["pred_logits"] = outputs_class[-1]  #取最后一层为主要的pred_logits

        if self.aux_loss:
            mask_embed = self.mask_embed(hs) # 6 * 1 *100*256 

            output_seg_masks = paddle.einsum("lbqc,bchw->lbqhw", mask_embed,
                                             mask_features) # 6 * 1 *100* 160*90  #100个instance
            out["pred_masks"] = output_seg_masks[-1]  #取最后一层为主要的pred_masks
            if self.mask_classification:  #其余层为辅助
                out['aux_outputs'] = [{
                    "pred_logits": a,
                    "pred_masks": b
...
class MaskFormerHead(nn.Layer):
    def __init__(self, input_shape, num_classes, transformer_in_feature='res5'):
        ...
        self.pixel_decoder = BasePixelDecoder(input_shape)  #单纯的FPN结构
    ...
    def forward(self, x):
        mask_features, transformer_encoder_features = self.pixel_decoder(x) #通过上采样得到mask #mask_features:[1, 256, 160, 90]
