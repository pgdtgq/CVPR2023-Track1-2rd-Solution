    ...
    def forward(self,
        ...
        for i, layer in enumerate(self.layers):
            ...
            output = layer(output, reference_points_input, memory,
                           ...
                           attn_mask, memory_mask, query_pos_embed, training = training)
...
class DINOTransformer(nn.Layer):
    ...
    def __init__(self,
        ...
        self.num_backbone_feat = len(backbone_feat_channels)
    ...
    def _get_encoder_input(self, feats, pad_mask=None):
        ...
        num_total_feats = len(feats)
        feats = feats[num_total_feats-self.num_backbone_feat:]
        if not self.use_neck:
    ...
    def forward(self, feats, pad_mask=None, gt_meta=None):
        ...
        memory = self.encoder(feat_flatten, spatial_shapes, level_start_index,
                              mask_flatten, lvl_pos_embed_flatten, valid_ratios, training = self.training)
        ...
        inter_feats, inter_ref_bboxes_unact = self.decoder(
            ...
            valid_ratios, attn_mask, mask_flatten, training = self.training)
        out_bboxes = []
        out_logits = []
