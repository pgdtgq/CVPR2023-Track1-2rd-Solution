...
class Block(nn.Layer):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 ...
                 window_size=None, attn_head_dim=None, window=False, lr=1.0, depth = 0):
        super().__init__()
...
class ViT(nn.Layer):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=80, embed_dim=768, depth=12,
        ...
        self.blocks = nn.LayerList([
            Block(
                ...
                window=((i + 1) % interval != 0), #((i + 1) % interval != 0)  不采用window
                depth = i)  
    ...
    def forward_features(self, x):
        ...
        # 0 1 2 3
        ops = [self.fpn1, self.fpn2, self.fpn3, self.fpn4]  #特征金字塔的结构
        ...
        #0:上采样两倍 1保持不变 2下采样2倍 3下采样4倍数
        return [tuple(det_features), all_features]

    def forward(self, inputs):
        x = inputs['image']
        x = self.forward_features(x)  #x的输出有两部分构成，第一部分FPN
        return x
...
if __name__=='__main__':
    model = ViT()
    inputs = paddle.randn((2,3,512,512))
    x = {}
    x['image'] = inputs
    output = model(x)